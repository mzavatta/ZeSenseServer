...
//Android emulator loopback inteface on the develoment machine
//10.0.2.2
//Android emulator incoming interface
//10.0.2.15

// Logging macros
#define LOGI(...) ((void)__android_log_print(ANDROID_LOG_INFO, "ze_coapserver", __VA_ARGS__))
#define LOGW(...) ((void)__android_log_print(ANDROID_LOG_WARN, "ze_coapserver", __VA_ARGS__))
...
void usage( const char *program, const char *version) {
  ...
  LOGW("%s v%s -- a small CoAP implementation\n"
